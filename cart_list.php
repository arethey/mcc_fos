     ...
     <div class="container">
         <div class="row">
             <div class="col-lg-8">
				...
				$total = 0;
				$get = $conn->query("SELECT *,c.id as cid, p.id as pid FROM cart c inner join product_list p on p.id = c.product_id ".$data);
                                 ...
                                 <div class="input-group mb-3">
                                     <div class="input-group-prepend">
                                         <button class="btn btn-outline-secondary qty-minus" type="button"
                                             ...
                                             data-pid="<?php echo $row['pid'] ?>"><span class="fa fa-minus"></button>
                                     </div>
                                     <input type="number" readonly value="<?php echo $row['qty'] ?>" min=1
                                         max="<?php echo $row['stocks'] ?>" class="form-control text-center" name="qty">
                                     <div class="input-group-prepend">
                                         <button class="btn btn-outline-secondary qty-plus" type="button" id=""
                                             data-id="<?php echo $row['cid'] ?>"
                                             data-pid="<?php echo $row['pid'] ?>"><span
                                                 class="fa fa-plus"></span></button>
                                     </div>
                                 </div>
                                 <button class="btn btn-sm btn-danger"
                                     onclick="removeItem(<?php echo $row['pid'].','.$row['cid'].','.$row['qty'] ?>)">Remove</button>
                             </div>
                     ...
                     <div class="card">
                         <div class="card-body">
                             ...
                             <div class="text-center">
                                 <button class="btn btn-block btn-outline-primary" type="button" id="checkout">Proceed
                                     to Checkout</button>
                             </div>
                         </div>
                     </div>
...
function update_qty(qty, id, pid, action) {
    start_load()
    $.ajax({
...
function removeItem(productId, cartId, qty) {
    start_load()
    $.ajax({
        url: 'admin/ajax.php?action=remove_cart',
        method: "POST",
        data: {
            productId,
            cartId,
